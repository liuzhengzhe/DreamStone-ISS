...
if __name__ == '__main__':
    ...
    df=torch.save(model.state_dict(),'dreamfusion.pt')
    
    path=glob.glob('../stage2/out/'+opt.source+'/model10.pt')[0]
    
    
    shutil.copy(path, 'model10.pt')
    shutil.copy(path.replace('model10.pt', 'c.npy'), 'c.npy')


    weight=torch.load('model10.pt')['model']
    df=torch.load('dreamfusion.pt')
    for key in weight.keys():
      if 'decoder' in key:
        df[key.replace('decoder.','sigma_net.')]=weight[key]
    for key in weight.keys():
      '''if 'generator.linear_4x' in key:
        df[key.replace('generator.linear_4x.','generator.linear_4.')]=weight[key]
      elif 'generator.linear_5x' in key:
        df[key.replace('generator.linear_5x.','generator.linear_5.')]=weight[key]
      elif 'generator.linear_6x' in key:
        df[key.replace('generator.linear_6x.','generator.linear_6.')]=weight[key]'''
      if 'generator' in key:
        df[key]=weight[key]

    torch.save(df, 'init.pt')
        
    
    
    model.load_state_dict(torch.load('init.pt'), strict=True)
    model.train()
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            max_epoch = 100 #np.ceil(opt.iters / len(train_loader)).astype(np.int32)
