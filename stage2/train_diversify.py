...
if __name__ == '__main__':
    ...
    text_cond = dict(text_embed = c_clip)
    prior_embeds = diffusion_prior.p_sample_loop((1, 512), text_cond = text_cond)
    
    condition=prior_embeds
    
    #print ('prior_embeds',condition)
    
    #print ('sim',1-torch.sum(c_clip*condition))
    
    #print (c0.norm(dim=-1, keepdim=True),condition.norm(dim=-1, keepdim=True) )
    condition0=c_clip+condition
    condition=condition0/condition0.norm(dim=-1, keepdim=True)
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            loss,clip1, clip2,dis = trainer.train_step(batch,c_clip, condition0, text, it)
