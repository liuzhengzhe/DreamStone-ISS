...
if __name__ == '__main__':
    ...
    parser.add_argument('--text', type=str, default='tulip',
                        help='Text')
    ...
    model_std = config.get_model(cfg, device=device, len_dataset=len(train_dataset))
    ...
    trainer = config.get_trainer(model, model_std, optimizer, cfg, device=device,
                                generator=generator)

    checkpoint_io = CheckpointIO(out_dir, model=model_std, optimizer=optimizer)
    try:
        load_dict = checkpoint_io.load(args.model, device=device)
    ...
    for param in trainer.model_std.parameters():
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            it += 1
            loss,maskloss, shape_loss = trainer.train_step(batch, c,c_std, it)
            logger.add_scalar('train/loss', loss, it)

            # Print output
            if print_every > 0 and (it % print_every) == 0:
                logger_py.info('[Epoch %02d] it=%03d, loss=%.4f, maskloss=%.4f,shapeloss=%.4f, time=%.4f'
                            % (epoch_it, it, loss, maskloss,shape_loss, time.time() - t0b))
                t0b = time.time()
            ...
            if it>0 and (checkpoint_every > 0 and (it % checkpoint_every) == 0):
                ...
                checkpoint_io.save('model'+str(it)+'.pt', epoch_it=epoch_it, it=it,
