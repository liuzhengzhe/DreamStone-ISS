...
def get_model(cfg, device=None, len_dataset=0, **kwargs):
    ...
    '''
    decoder_shape = cfg['model']['decoder_shape']
    decoder_color = cfg['model']['decoder_color']
    ...
    decoder_shape = models.decoder_dict[decoder_shape](
        dim=dim, c_dim=c_dim, **decoder_kwargs
    )

    decoder_color = models.decoder_dict[decoder_color](
        dim=dim, c_dim=c_dim, **decoder_kwargs
    )
    ...
    model = models.DVR(
        decoder_shape, decoder_color,generator, encoder=encoder, device=device,
