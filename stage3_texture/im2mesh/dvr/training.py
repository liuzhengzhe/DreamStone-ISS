...
class Trainer(BaseTrainer):
    ...
    def train_step(self, data,c,c_std, it=None):
        ...
        loss, maskloss = self.compute_loss(data,c,c_std, it=it)
        # loss.backward(retain_graph=True)
        loss.backward()
        check_weights(self.model.state_dict())
        self.optimizer.step()

        return loss.item(), maskloss.item()
    ...
    def compute_loss(self, data,c, c_std,eval_mode=False, it=None):
        ...
        for rrr in range(3):
          ...
          empty_loss = torch.mean(rgb_pred)
          ...
          #mask_pred_reshape[torch.where(mask_pred_reshape==0)]=1
        ...
        if eval_mode:
          ...
          return loss['loss'], mask_255_loss
