...
class Trainer(BaseTrainer):
    ...
    def train_step(self, data, it=None):
        ...
        loss,mask_255_loss,regress_loss = self.compute_loss(data, it=it)
        # loss.backward(retain_graph=True)
        loss.backward()
        check_weights(self.model.state_dict())
        self.optimizer.step()

        return loss.item(),mask_255_loss.item(),regress_loss.item()
    ...
    def compute_loss(self, data, eval_mode=False, it=None):
        ...
        assert(((h, w) == mask_img.shape[2:4]) and
               (patch_size > 0) and
               (n_points > 0))
        #print (mask_img.shape, h,w, n_points)
        ...
        #    p_world_hat_sparse, mask_pred_sparse, normals, c, c_std) 

        (p_world_hat, rgb_pred, logits_occupancy, logits_freespace, mask_pred,
            p_world_hat_sparse, mask_pred_sparse, normals, c, c_std) = self.model(
        ...
        mask_255_loss=torch.mean(torch.abs(mask_255*mask_pred_inv-rgb_pred*mask_pred_inv))*30
        ...
        regress_loss=torch.mean(torch.abs(c-c_std))*2000.0
        #loss['loss']/=10.0
        ...
        if eval_mode:
          ...
          return loss['loss'], mask_255_loss, regress_loss
