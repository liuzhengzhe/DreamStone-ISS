...
class DVR(nn.Module):
    ...
    def forward(self, pixels, p_occupancy, p_freespace, inputs, camera_mat,
        ...
        random_noise = torch.randn(c.shape).cuda()  
        random_noise = random_noise/random_noise.norm(dim=-1, keepdim=True) 
        fake_txt_features= c*(1-0.75) + random_noise*0.75  
        c  =  fake_txt_features / fake_txt_features.norm(dim=-1, keepdim=True) 
        c=self.generator(c)
        #return c, c_std
        ...
        p_world, mask_pred, mask_zero_occupied = \
            self.pixels_to_world(pixels, camera_mat,
                                 world_mat, scale_mat, c_std, it)
        rgb_pred = self.decode_color(p_world, c=c_std)

        # eval occ at sampled p
        logits_occupancy = self.decode(p_occupancy, c=c_std).logits

        # eval freespace at p and
        # fill in predicted world points
        p_freespace[mask_pred] = p_world[mask_pred].detach()
        logits_freespace = self.decode(p_freespace, c=c_std,).logits

        if calc_normals:
            normals = self.get_normals(p_world.detach(), mask_pred, c=c_std)
        else:
            normals = None
        ...
        if sparse_depth is not None:
            ...
            p_world_sparse, mask_pred_sparse, _ = self.pixels_to_world(
                p, camera_mat, world_mat, scale_mat, c_std, it)
        else:
            p_world_sparse, mask_pred_sparse = None, None

        return (p_world, rgb_pred, logits_occupancy, logits_freespace,
                mask_pred, p_world_sparse, mask_pred_sparse, normals, c, c_std.detach())
    ...
    def encode_inputs(self, inputs):
        ...
        if self.encoder is not None:
            ...
            c = self.clip_model.encode_image(inputs).float() #self.encoder(inputs)
        else:
