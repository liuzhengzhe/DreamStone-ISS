...
class Generator3D(object):
    ...
    def generate_mesh(self, data, return_stats=True):
        ...
        #image = self.preprocess(Image.open("/mnt/sdc/lzz/diff_ori_test_opt/media/demo/choy_renderings/f32cfab76f75cbebc6ced373b157c8ba_input.jpg")).unsqueeze(0).cuda()
        #c=self.clip_model.encode_image(image).float()
        #print (c)
        ...
        #text = clip.tokenize("a black table with white legs.").to(device) #"ferry boat watercraft ship").to(device)
        #c = self.clip_model.encode_text(text).float()

        #np.save('/mnt/sdc/lzz/diff_test_mapper_e2e_2stage4/c_init.npy',c.detach().cpu().numpy())


        #idx=torch.from_numpy(np.load('../bank/idx.npy')).float().cuda()#[6:7,:]

        #inv_idx=torch.ones(idx.shape).cuda()-idx
        #rand=torch.rand(1,512).cuda()
        


        c=torch.from_numpy(np.load('../bank/mean.npy')).float().cuda()#[6:7,:]
        #print (torch.unique(c), torch.unique(rand))
        
        #c=c*idx+rand*inv_idx
        ...
        #np.save('/mnt/sdc/lzz/diff_test_mapper_e2e_dvr_space/c_init.npy',c.detach().cpu().numpy())



        mesh, soma_array, grid = self.generate_from_latent(c_std, stats_dict=stats_dict,
                                         data=data, **kwargs)
