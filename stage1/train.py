...
if __name__ == '__main__':
    ...
    #model.load_state_dict(torch.load('../bigmapper_test_2stage2_disc/model.pt')['model'],strict=False) #the true init one
    #model.load_state_dict(torch.load('/mnt/sdc/lzz/differentiable_volumetric_rendering_original/ours_combined-af2bce07.pt')['model'],strict=False)
    ...
    optimizer = optim.Adam([{"params":model.generator.parameters()},{"params":model.decoder.parameters()}],  lr=lr*1)
    ...
    try:
        load_dict = checkpoint_io.load('/mnt/sdc/lzz/stage2/model.pt', device=device)
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            it += 1
            loss,mask_loss,regress_loss = trainer.train_step(batch, it)
            logger.add_scalar('train/loss', loss, it)

            # Print output
            if print_every > 0 and (it % print_every) == 0:
                logger_py.info('[Epoch %02d] it=%03d, loss=%.4f,mask_255_loss=%.4f, regress_loss=%.4f, time=%.4f'
                            % (epoch_it, it, loss, mask_loss,regress_loss, time.time() - t0b))
                t0b = time.time()
