...
class DVR(nn.Module):
    ...
    def forward(self, pixels, p_occupancy, p_freespace, inputs, camera_mat,
        ...
        '''random_noise = torch.randn(c.shape).cuda()  # random Gaussian
        random_noise = random_noise/random_noise.norm(dim=-1, keepdim=True) # Normalize to sphere
        fake_txt_features= c*(1-0.75) + random_noise*0.75  # 0.75 is hyper-parameter, similar to what you have done
        c  =  fake_txt_features / fake_txt_features.norm(dim=-1, keepdim=True) # normalize to sphere'''
    ...
    def decode(self, p, c=None, **kwargs):
        ...
        print ('1', logits.shape, torch.unique(logits))
        p_r = dist.Bernoulli(logits=logits)
        print ('2', p_r.logits.shape, torch.unique(p_r.logits))
        return p_r
    ...
    def pixels_to_world(self, pixels, camera_mat, world_mat, scale_mat, c,
        ...
        #print (pixels.shape, camera_mat.shape, world_mat.shape)
