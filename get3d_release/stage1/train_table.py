...
class customData(Dataset):
    ...
    def __init__(self, dataset = '', data_transforms=None, loader = default_loader):
        self.img_name=glob.glob('save_inference_results/shapenet_table/inference/interpolation/*/inter_img.jpg')#[:30]
...
for epoch in range(0,epochs):
	for clip_image, g_gt,c_gt in dataloders:
		...
		if epoch % 2 == 0: 
			torch.save(model.state_dict(), 'model_table_'+str(epoch)+'.pt')
			try:
				os.remove('model_table_'+str(epoch-4)+'.pt')
			except:
				pass