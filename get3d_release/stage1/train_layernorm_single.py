...
class customData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        f=np.load(self.img_name[item]+'/feat.npy')
        g=f[:512]
        c=f[512:]
        #print (g.shape)
...
model.load_state_dict(torch.load('model_32_no_noise878.pt'), strict=True)
model.train()
...
for epoch in range(3,epochs):
	for clip_image, g_gt,c_gt in dataloders:
		...
		if epoch % 2 == 0: 
			torch.save(model.state_dict(), 'model_32_no_noise'+str(epoch)+'.pt')
			try:
				os.remove('model_32_no_noise'+str(epoch-4)+'.pt')
			except:
				pass