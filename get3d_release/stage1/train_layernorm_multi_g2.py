...
class customData(Dataset):
    def __init__(self, dataset = '', data_transforms=None, loader = default_loader):
        self.img_name=glob.glob('save_inference_results/shapenet_chair/inference/interpolation/*')#[:30]
...
for epoch in range(3,epochs):
	for clip_image, g_gt,c_gt in dataloders:
		...
		g1, g2, c=model(clip_feature.detach())
		...
		g1loss = torch.mean(torch.abs(g1-g_gt[:,:512*11]))
		g2loss = torch.mean(torch.abs(g2-g_gt[:,512*11:])) #torch.sum(1-clip_criterion(pred_feature, gt.detach()))*40 
		closs = torch.mean(torch.abs(c-c_gt))
		print ('loss', g1loss, g2loss, closs, epoch)
		loss=g1loss+g2loss+closs
		loss.backward()
		optimizer.step()
		running_loss += loss.item()
		#print(f'[{epoch + 1}, {i + 1:5d}] loss: {running_loss/i :.3f}')
		if epoch % 2 == 0: 
			torch.save(model.state_dict(), 'model_batch_1_'+str(epoch)+'.pt')
			try:
				os.remove('model_batch_1_'+str(epoch-4)+'.pt')
			except:
				pass