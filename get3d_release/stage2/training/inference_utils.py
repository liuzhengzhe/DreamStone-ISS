            ...
            for i_tex, ws_tex in enumerate(ws_tex_list):
                ...
                if 1: #gen_mesh:
                    ...
                    for mesh_v, mesh_f, all_uvs, all_mesh_tex_idx, tex_map in zip(*generated_mesh):
                        ...
                        #print (np.unique(img),'img')
                        #img[np.where(img<5)]=255
                        #print (np.unique(img),'img')
                        #print (os.path.join(save_path, '%02d_%02d.png' % (i_geo, i_tex)))
...
def save_visualization_for_interpolation(
    ...
    with torch.no_grad():
        ...
        for i in range(1): #len(select_geo_codes) - 1):
            ...
            text='a table with square wood legs. The tabletop is blue. '
            print (text)
            device='cuda'
            import clip
            ...
            mapper.load_state_dict(torch.load('../../GET3Dold/model_table_222.pt'), strict=True)
            mapper.train()
            ...
            geo_z=g.repeat(1,22,1)#[0,:,:] #torch.cat((g1,g2),1)
            tex_z=c.repeat(1,9,1)#[0,:,:]


            gen_swap(
                geo_z, tex_z, camera_list[0], generator,
                save_path='result.png', gen_mesh=gen_mesh
            )


            generated_mesh = generator.generate_3d_mesh(
                geo_z=geo_z, tex_z=tex_z, c=None, truncation_psi=0.7,
                use_style_mixing=0,use_mapping=0)
            for mesh_v, mesh_f, all_uvs, all_mesh_tex_idx, tex_map in zip(*generated_mesh):
                savemeshtes2(
                    mesh_v.data.cpu().numpy(),
                    all_uvs.data.cpu().numpy(),
                    mesh_f.data.cpu().numpy(),
                    all_mesh_tex_idx.data.cpu().numpy(),
                    os.path.join('mesh/0.obj') #, '%07d.obj' % (save_mesh_idx))
                )
                lo, hi = (-1, 1)
                img = np.asarray(tex_map.permute(1, 2, 0).data.cpu().numpy(), dtype=np.float32)
                img = (img - lo) * (255 / (hi - lo))
                ...
                #img[np.where(img==0)]=255
                PIL.Image.fromarray(np.ascontiguousarray(img[::-1, :, :]), 'RGB').save(
                    os.path.join('mesh/0.png')) #, '%07d.png' % (save_mesh_idx)))
                #save_mesh_idx += 1
