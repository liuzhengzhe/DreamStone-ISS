...
def inference(
    ...
    print (grid_z[0].shape, grid_tex_z[0].shape, '1')
    ...
    mapper.eval()
    grid_z, grid_tex_z=mapper(text_features.detach().float())
    print (grid_z.shape, grid_tex_z.shape, '2')
    
    grid_z=grid_z.unsqueeze(0).unsqueeze(0)
    grid_tex_z=grid_tex_z.unsqueeze(0).unsqueeze(0)
    
    grid_z[:]=torch.from_numpy(np.load('g.npy')).cuda().unsqueeze(0).unsqueeze(0)
    grid_tex_z[:]=torch.from_numpy(np.load('c.npy')).cuda().unsqueeze(0).unsqueeze(0)
    print (grid_z.shape, grid_tex_z.shape, '3')
    
    #grid_z=grid_z.split(1)
    #grid_tex_z=grid_tex_z.split(1)
    ...
    if 1: #inference_to_generate_textured_mesh:
    ...
    if 0: #inference_save_interpolation:
