...
def training_loop(
    ...
    optimizer = optim.Adam([{"params": mapper.parameters(), "lr": 1e-6}])  #std: 1e-6 #, {"params": c_clip, "lr": lr*1}]) #model.decoder_color.parameters(), lr=lr*1) #([{"params": model.encoder.parameters(), "lr": lr*1}, {"params": model.decoder.parameters(), "lr": lr*01}], lr=lr)
    ...
    while True:
        ...
        for i in range(20):
            ...
            #gen_img[torch.where(mask==0)]*=(-1)
            #print ('genimg.shape',gen_img.shape,gen_sdf.shape)
            ...
            #print (np.unique(img))
            if i==0:
                cv2.imwrite('result/im'+str(len(glob.glob('result/im*.png')))+'.png',cv2.resize(img,(256,256)))

            #mask=gen_img[0,3,:,:]
            #mask=mask.detach().cpu().numpy()*255
            #cv2.imwrite('result/mask'+str(len(glob.glob('result/mask*.png')))+'.png',mask)

            img=torch.nn.functional.interpolate(gen_img[:,:3,:,:]/2.2/0.27, (224,224))
            
            #img[torch.where(img==0)]=
