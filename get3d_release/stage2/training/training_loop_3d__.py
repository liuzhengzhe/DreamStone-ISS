...
def training_loop(
    ...
    text_features_ori = clipmodel.encode_text(text)
    
    for param in clipmodel.parameters():
      param.requires_grad=False

    text_features = text_features_ori / text_features_ori.norm(dim=-1, keepdim=True)


    #import train_layernorm_multi_g2
    mapper = generator_mapper(4,64).cuda()
    mapper.load_state_dict(torch.load(stage1_model), strict=True)
    mapper.train()


    import torch.optim as optim
    optimizer = optim.Adam([{"params": mapper.parameters(), "lr": 1e-5}])  #std: 1e-6 #, {"params": c_clip, "lr": lr*1}]) #model.decoder_color.parameters(), lr=lr*1) #([{"params": model.encoder.parameters(), "lr": lr*1}, {"params": model.decoder.parameters(), "lr": lr*01}], lr=lr)
    ...
    while True:
        ...
        g, c=mapper(text_features.detach().float())
        #g1=torch.reshape(g1, (1,11,512))
        ##g2=torch.reshape(g2, (1,11,512))
        #c=torch.reshape(c, (1,9,512))

        gen_z=g.repeat(1,22,1) #torch.cat((g1,g2),1)
        gen_c=c.repeat(1,9,1)
        ...
        for i in range(20):
            ...
            mask=gen_img[0,3:,:,:].repeat(1,3,1,1)
            
            gen_img[torch.where(mask==0)]+=1.95   #-1.4, 1
            
            
            gen_img=torch.clamp(gen_img, -1.6, 1.2)
            #print ('genimg.shape',gen_img.shape, torch.unique(gen_img[:,:3,:,:]) )
            #print ('0',torch.unique(gen_img[:,:3,:,:][torch.where(mask==0)]))
            #print ('1',torch.unique(gen_img[:,:3,:,:][torch.where(mask==1)]))
            
            img=torch.permute(gen_img[0,:3,:,:],(1,2,0))
            #print (img.shape, torch.unique(gen_img))
            img=img.detach().cpu().numpy()[:,:,::-1]
            
            #print ('1',np.unique(img))

            
            img+=1.4
            img*=80
            #print ('save',np.unique(img))
            if i==0:
                cv2.imwrite('result/im'+str(it)+'.png',cv2.resize(img,(256,256)))

            #mask=gen_img[0,3,:,:]
            #mask=mask.detach().cpu().numpy()*255
            #cv2.imwrite('result/mask'+str(len(glob.glob('result/mask*.png')))+'.png',mask)
            
            
            gen_img/=4 #(2.4/0.81)
            gen_img-=0.17
            
            #gen_img=torch.clamp(gen_img, -0.45, 0.26)
            print ('clip',torch.unique(gen_img))
            

            img=torch.nn.functional.interpolate(gen_img[:,:3,:,:], (224,224))
            

            image_features = clipmodel.encode_image(img)


            
            los=torch.sum((1-clip_criterion(image_features, text_features_ori.detach())))
        ...
        print ('loss', los_s.item(), min_it)
        los_s.backward()

        optimizer.step()
        #@print (gen_img.shape, torch.unique(gen_img))
        continue
